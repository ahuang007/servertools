...
<script>
    ...
    var tbname = "tb_user_pay_income_record";
    ...
    export default {
        ...
        methods: {
            onSubmit() {
                ...
                var param1 = this.GLOBAL.trim(this.form.username);
                if (param1 == "") { alert("参数1错误!"); return (false); }
                var param2 = this.GLOBAL.trim(this.form.record_id);
                if (param2 == "") { alert("参数2错误!"); return (false); }
                var url = "http://"+this.GLOBAL.srvIp+":"+this.GLOBAL.srvPort+"/MysqlQuery?";
                url = url + "ip=" + ip + "&cmd=query&tbname=" + tbname + "&param1=username&param2=" + param1;
                url = url + "&param3=record_id&param4=" + param2;
